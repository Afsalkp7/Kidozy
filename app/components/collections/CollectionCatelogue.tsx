  ...
  return (
    ...
    >
      <div className="aspect-[5/5] overflow-hidden relative">
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent transition-opacity duration-300 flex items-end p-6 
  ...
  return (
    <section className="py-8">
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4">
        {items.map((item) => (
          <CatalogItem key={item.id} image={item.image} />
        ))}
      </div>
    </section>
  );
